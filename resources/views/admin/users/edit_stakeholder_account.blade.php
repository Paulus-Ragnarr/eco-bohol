        ...
        <div class="grid md:grid-cols-2 grid-rows-1 gap-5">
            <div>
                ...
                <div class="mt-3 p-6 max-w-4xl border border-gray-200 rounded-lg shadow">
                    ...
                    <form
                        action={{ '/admin/account-settings/edit-stakeholder-account' }}{{ $user ? '?user_id=' . $user->user_id : '' }}
                        method="POST">
                        @method('patch')
                        @csrf
                        ...
                        <div class="flex flex-col mb-4">
                            <label class="block text-gray-700 font-bold mb-2" for="org_name">Organization Name</label>
                            <input class="common-input rounded px-4 py-2 w-full" type="text" name="org_name"
                                id="org_name" placeholder="Enter your organization name"
                                value="{{ old('org_name', $stakeholder ? $stakeholder->org_name : null) }}" required>
                            @error('org_name')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="flex flex-col mb-4">
                            <label class="block text-gray-700 font-bold mb-2" for="stakeholder_type">Organization
                                Type</label>
                            <select class="common-input rounded px-4 py-2 w-full" name="stakeholder_type"
                                id="stakeholder_type" required>
                                <option value="">Select a Organization Type</option>
                                <option value="Non Government Organization"
                                    {{ $stakeholder ? ($stakeholder->stakeholder_type == 'Non Government Organization' ? 'selected' : null) : null }}>
                                    Non Government Organization</option>
                                <option value="Local Government Unit"
                                    {{ $stakeholder ? ($stakeholder->stakeholder_type == 'Local Government Unit' ? 'selected' : null) : null }}>
                                    Local Government Unit</option>
                                <option value="Governmant Agency"
                                    {{ $stakeholder ? ($stakeholder->stakeholder_type == 'Governmant Agency' ? 'selected' : null) : null }}>
                                    Governmant Agency</option>
                            </select>
                            @error('org_type')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
            ...
            <div>
                ...
                <div class="mt-3 p-6 max-w-4xl border border-gray-200 rounded-lg shadow">
                    ...
                    <form
                        action={{ '/admin/account-settings/edit-stakeholder-password' }}{{ $user ? '?user_id=' . $user->user_id : '' }}
                        method="POST">
                        @method('patch')
                        @csrf
                        <div class="flex flex-col mb-4">
