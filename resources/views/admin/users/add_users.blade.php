...
@section('content')
    ...
    <div class="common-background flex-1 p-5 h-screen">
        ...
        <form action="/admin/manage-users/add" method="POST">
            @csrf
                ...
                <div class="flex flex-col md:flex-row md:items-center">
                    ...
                    <div class="grid lg:grid-cols-3 md:grid-cols-2">
                        {{-- <div class="md:ml-10 flex flex-col mb-2 md:mb-0">
                            ...
                            <input class="common-input" type="text" name="office" value="{{ old('office') }}" required>
                            @error('office')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        </div> --}}
                        <div class="md:ml-10 flex flex-col mb-2 md:mb-0">
                            ...
                            <input class="common-input" type="text" name="position" value="{{ old('position') }}"
                                required>
                            @error('position')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="md:ml-10 flex flex-col mb-2 md:mb-0">
                            ...
                            <select class="common-input" type="text" name="user_role" required>
                                <option value="">Select a Role</option>
                                <option value="admin" {{ old('user_role') == 'admin' ? 'selected' : '' }}>Admin</option>
                                <option value="officer" {{ old('user_role') == 'officer' ? 'selected' : '' }}>Officer
                                </option>
                                <option value="researcher" {{ old('user_role') == 'researcher' ? 'selected' : '' }}>
                                    Researcher</option>
                            </select>
                            @error('user_role')
                                <span class="text-red-500">{{ $message }}</span>
