    ...
    <div class="common-background flex-1 pt-5 px-5 pb-40 overflow-y-auto space-y-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-3 pt-5 pb-10">
            <div class="p-2 bg-gray-100 rounded-xl hover:bg-green-50">
                ...
                <span class="font-bold">{{ $pending->user_id }}</span>
            </div>
            <div class="p-2 bg-gray-100 rounded-xl hover:bg-green-50">
                <span>Stakeholder Name:</span>
                <span class="font-bold">{{ $pending->name }}</span>
            </div>
            <div class="p-2 bg-gray-100 rounded-xl hover:bg-green-50">
                <span>Email:</span>
                <span class="font-bold">{{ $pending->email }}</span>
            </div>
            <div class="p-2 bg-gray-100 rounded-xl hover:bg-green-50">
                <span>Contact:</span>
                <span class="font-bold">{{ $pending->user_contact }}</span>
            </div>
            <div class="p-2 bg-gray-100 rounded-xl hover:bg-green-50">
                <span>Organization Name:</span>
                <span class="font-bold">{{ $pending->stakeholder->org_name }}</span>
            </div>
            <div class="p-2 bg-gray-100 rounded-xl hover:bg-green-50 ">
                <span>Status:</span>
                <span
                    class="font-bold {{ $pending->status == 'Active' ? 'text-green-600' : 'text-red-600' }}"">{{ $pending->status }}</span>
            </div>
            <div class="p-2 bg-gray-100 rounded-xl hover:bg-green-50">
                <span>Type:</span>
                <span class="font-bold">{{ $pending->stakeholder->stakeholder_type }}</span>
            </div>
            <div class="p-2 bg-gray-100 rounded-xl hover:bg-green-50">
                <span>Endorsement Letter:</span>
                <br>
                <span class="font-bold flex flex-col">
                    @foreach ($pending->stakeholder->endorsement_letters as $attachment)
