        ...
        <div class="grid md:grid-cols-2 grid-rows-1 gap-5">
            <div>
                ...
                <div class="mt-3 p-6 max-w-4xl border border-gray-200 rounded-lg shadow">
                    ...
                    <form
                        action={{ '/admin/account-settings/edit-manager-account' }}{{ $user ? '?user_id=' . $user->user_id : '' }}
                        method="POST">
                        @method('patch')
                        @csrf
                        ...
                        <div class="flex flex-col mb-4">
                            ...
                            <input class="common-input rounded px-4 py-2 w-full" type="text" name="org_name"
                                ...
                                value="{{ old('org_name', $manager ? $manager->org_name : null) }}" required>
                            @error('org_name')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="flex flex-col mb-4">
                            <label class="block text-gray-700 font-bold mb-2" for="org_type">Organization Type</label>
                            <select class="common-input rounded px-4 py-2 w-full" name="org_type" id="org_type" required>
                                <option value="">Select a Organization Type</option>
                                <option value="PO"
                                    {{ $manager ? ($manager->org_type == 'PO' ? 'selected' : null) : null }}>
                                    People's Organization</option>
                                <option value="BLGU"
                                    {{ $manager ? ($manager->org_type == 'BLGU' ? 'selected' : null) : null }}>Barangay
                                    Local
            ...
            <div>
                ...
                <div class="mt-3 p-6 max-w-4xl border border-gray-200 rounded-lg shadow">
                    ...
                    <form
                        action={{ '/admin/account-settings/edit-manager-password' }}{{ $user ? '?user_id=' . $user->user_id : '' }}
                        method="POST">
                        @method('patch')
                        @csrf
                        <div class="flex flex-col mb-4">
